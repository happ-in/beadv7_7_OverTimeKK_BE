 ...
 * "아직 만료 안 된 멤버 수"로 계산한다. 반납(release) 호출을 놓쳐도 만료시각이 지나면
 * 자동으로 카운트에서 빠지기 때문에, 카운터가 새서 대기열이 영구 정지되는 문제가 구조적으로 없다.
 */
@Component
public class LoginQueueService {
     ...
     * 아니면 대기열 ZSET에 등록한다. 조회와 등록을 하나로 묶어 동시 요청이 정원을 초과해
     * 통과하는 레이스를 막는다.
     */
    ...
    /**
     * 만료된 활성 멤버를 먼저 청소하고, 남은 빈 자리 수만큼 대기열에서
     * 가장 오래 기다린(score가 가장 작은) 순서대로 꺼내 활성 ZSET으로 옮긴다.
     */
